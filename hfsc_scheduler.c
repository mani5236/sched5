/********************************************************************
 * HFSC Scheduler - Core Implementation v1.7
 * 
 * CRITICAL FIXES in v1.7:
 * - Fix 1 (v1.6): Parent VT update on child deactivation with propagation
 * - Fix 2 (v1.7): VT refresh in hfsc_select_ls before comparison
 ********************************************************************/
...
/* CRITICAL FIX v1.4: Get average VT across all active branches at same level */
...
static void hfsc_activate_class(hfsc_scheduler_t *sched, hfsc_class_t *cl)
{
    ...
    if (cl->fsc.m1 > 0 || cl->fsc.m2 > 0) {
        ...
        if (cl->parent && cl->parent->state == HFSC_CLASS_ACTIVE) {
            /* CRITICAL FIX v1.4: Sync with average VT across all active siblings */
...
/* CRITICAL FIX v1.6: Reset VT tracking on deactivation */
static void hfsc_deactivate_class(hfsc_scheduler_t *sched, hfsc_class_t *cl)
{
    ...
    if (cl->parent) {
        ...
        if (!has_active_child) {
            ...
            /* CRITICAL FIX v1.6: Always update parent VT when a child deactivates.
             * Previously this only ran when fsc.m1==0, causing parent VT to go
             * stale and always win LS selection, starving other branches. */
            uint64_t cur_time_deact = hfsc_get_time();
            if (cl->parent->fsc.m1 > 0 || cl->parent->fsc.m2 > 0) {
                hfsc_update_sc(&cl->parent->cl_fsc, &cl->parent->fsc,
                               cur_time_deact, cl->parent->total, sched->tsc_hz);
                cl->parent->cl_vt = hfsc_sc_y2x(&cl->parent->cl_fsc, cl->parent->total);
                cl->parent->cl_vt += cl->parent->cl_vtoff;
            } else {
                if (cl->parent->cl_cvtmin != UINT64_MAX && cl->parent->cl_cvtmax != 0) {
            ...
            /* Propagate VT update up the hierarchy */
            for (hfsc_class_t *anc = cl->parent->parent; anc != NULL; anc = anc->parent) {
                if (anc->state != HFSC_CLASS_ACTIVE) break;
                hfsc_update_vt_minmax(anc);
                if (anc->fsc.m1 > 0 || anc->fsc.m2 > 0) {
                    hfsc_update_sc(&anc->cl_fsc, &anc->fsc,
                                   cur_time_deact, anc->total, sched->tsc_hz);
                    anc->cl_vt = hfsc_sc_y2x(&anc->cl_fsc, anc->total);
                    anc->cl_vt += anc->cl_vtoff;
                } else {
                    if (anc->cl_cvtmin != UINT64_MAX && anc->cl_cvtmax != 0) {
                        anc->cl_vt = (anc->cl_cvtmin + anc->cl_cvtmax) / 2;
                    }
                }
            }
        }
...
int hfsc_init(hfsc_scheduler_t **sched_out)
{
    ...
    HFSC_LOG(INFO, "HFSC scheduler v1.7 initialized (TSC=%lu Hz)\n", sched->tsc_hz);
